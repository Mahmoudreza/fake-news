...
<div id="mturk-intro" class="well">
    ...
    <br><br>
    We are a team of researchers from <a href = "http://socialnetworks.mpi-sws.org/" target="_blank"> Social Computing Research Group </a> at <a href = "http://mpi-sws.org" target="_blank"> Max Planck Institute for Software Systems</a> and the <a href = "https://www.umd.edu/" target="_blank"> University of Maryland</a>.

    </p>
    <p>
In this survey, you will be shown 52 claims, which are spreading or have spread over social media. We would like to evaluate the "truth value" of each claim based on your judgment. Your task is to read each claim and label it as either ‘True’, ‘Mostly True’, ‘Mixture’, ‘Mostly False’, or ‘False’. 
...
<br>
<!-- <p style="color:red;font-size:50px">Bonus Reward Rules:</p> -->
<h2 style="color:blue;">Bonus Reward Rules:</h2>

<ul style="list-style-type:circle">
  <li><P>In addition to the amount promised for the task, for each of your judgements which <font color="blue">CORRECTLY</font> match the actual status of the claims, we will pay you 5 cents as the bonus. 
    <br>For example, if you judge a claim to be <font color="green">‘True’</font> and the claim is actually <font color="green">True</font>, then you'll get 5 cents for the claim.
</P></li>
  <li><P>If your judgment for each question <font color="red">takes more than 15 seconds</font>, then there would not be any bonus, even if you answer the question correctly.</P></li>
  <li><P>The number of correct judgments would be shown at the end of the task.</P></li>
</ul> 
...
<p> The task consists of 52 claims, one per page. You need to give your judgment for all of them in order to complete the task. </p>
...
function judge($workerid, $page, $npages, $tweet_obj) {
    ...
    list($tweet_id, $sname, $tweet_text, $tweet_link, $tweet_gt) = $tweet_obj;
...
function countdown() {
    ...
    if (seconds == 1) {
      ...
      temp.innerHTML = "For this message you won't earn bonus! \n Please answer the qeustion and go to the next page.";
      return;
    }
 
    seconds--;
    temp = document.getElementById('countdown');
    temp.innerHTML = seconds + 's';
...
<b style = "color:black" >Claim: </b>
        ...
        if ($is_answered == 0){
            ...
            echo('<legend>Please give your rating for this claim.
                </legend>');
            ...
            echo('<input type="radio" name="a" id="a5" value="5" onClick="radio_btn_clicked(\'a5\')">');
            echo('<div id = "t5_text">True</div>');            
            echo('</label>');


            echo('<label class="radio">');
            echo('<input type="radio" name="a" id="a4" value="4" onClick="radio_btn_clicked(\'a4\')">');
            echo('<div id = "t4_text">Mostly True</div>');            
            echo('</label>');

            echo('<label class="radio">');
            echo('<input type="radio" name="a" id="a3" value="3" onClick="radio_btn_clicked(\'a3\')">');
            echo('<div id = "t3_text">Mixture</div>');            
            echo('</label>');
            ...
            echo('<legend>Please give your rating for this claim. </legend>');
            ...
            if ($given_answer_a == 5){
                ...
                echo('<input type="radio" name="a" id="a5" value="6" disabled>');
            }
            echo('<div id = "t5_text">True</div>');
            echo('</label>');
            ...
            if ($given_answer_a == 4){
                ...
                echo('<input type="radio" name="a" id="a4" value="5" disabled>');
            }
            echo('<div id = "t4_text">Mostly True</div>');
            echo('</label>');
            ...
            echo('<div id = "t2_text">Moslty False</div>');
            echo('</label>');
    ...
    <?php
        ...
        if ($page <= $npages){
        $lastpage=0;
        echo '<button  id = "btn_submit" class="btn" onClick="sendQueryWithAnswer('. $page_next .','. $workerid . ','. $tweet_gt . ');" disabled>Continue to next Claim</button>';

        }
        else{
            $lastpage=1;
            echo '<button  id = "btn_submit" class="btn" onClick="sendQueryWithAnswerLast('. $page_next .','. $workerid . ','. $tweet_gt . ');" disabled>Continue to next Claim</button>';
        }
...
function finish($workerid, $total_responses, $bonus) {
...
<div class="well">
    <center>
        <h2>Thank You!</h2>
        <p class="lead">
        You answered <?= $bonus ?> number of claims correctly! You will earn the correspondent bonus!
        </p>
...
if (2 <= $page and $page <= $npages + 2) {
    ...
    if($page <= $npages+1){
        list($tweet_id, $sname, $tweet_text, $tweet_link, $tweet_gt) = $set[$page - 2];
    }
    if($page == $npages +2){
        list($tweet_id, $sname, $tweet_text, $tweet_link, $tweet_gt) = $set[$page - 3];
    ...
    if ($is_answered == 0){
        ...
        survey_correct_response($workerid, $tweet_id, $ra, $tweet_gt);
        survey_response($workerid, $tweet_id, $ra, $rb, $rc, $rd, $text);
...
if ($page == 0) {
    ...
    if($page <= $npages){
        ...
        $tweet_obj = [$tweet_id+1, $sname, $tweet_text, $tweet_link, $tweet_gt];
    }
    ...
    if ($total_responses == count($set)+1) {
        list($bonus) = bonus_computing($workerid); 
        finish($workerid, $total_responses, $bonus);
...
function countdown() {
    ...
    if (seconds == 1) {
      temp = document.getElementById('countdown');
      temp.innerHTML = "For this message you won't earn bonus! <br> Please answer the qeustion and go to the next";
...
function sendQueryWithAnswer(p, w, gt)
{
...
function sendQueryWithAnswerLast(p, w, gt)
{
