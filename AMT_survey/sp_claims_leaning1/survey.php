...
<div id="mturk-intro" class="well">
    ...
    <p>
In this survey, you will be shown 52 claims, which are spreading or have spread over social media. We would like to evaluate the "POLITICAL LEANING" of each claim based on your judgment. Your task is to read each claim and label it as either ‘Liberal’ or ‘Conservative’. 
<br>
<li>By Conservative we mean whether the claim sounds like something that you think the Republican party would be more likely to say.</li>
<li>By Liberal we mean whether the claim sounds like something that you think the Democratic party would be more likely to say.</li>
<br>
<p>
In order to do the survey, you need to be familiar with the <a href = "https://en.wikipedia.org/wiki/Democratic_Party_presidential_debates,_2016" target="_blank">Democratic</a> and <a href = "https://en.wikipedia.org/wiki/Republican_Party_presidential_debates,_2016" target="_blank">Republican</a> parties. If you are not familiar with these parties, please click on the above links to access their Wikipedia descriptions.
</p>

<h3>There are no right or wrong answers to these questions, we are interested in your opinions. Please answer as honestly as possible.</h3>
...
function judge($workerid, $page, $npages, $tweet_obj) {
    ...
    ?>
        ...
        if ($is_answered == 0){
            ...
            echo('<legend>What is the leaning of the claim? </legend>');
            echo('<label class="radio">');
            if ($given_answer_a == 1){
    ...
    <?php
        ...
        if ($page <= $npages){
        $lastpage=0;
        echo '<button  id = "btn_submit" class="btn" onClick="sendQueryWithAnswer('. $page_next .','. $workerid . ');" disabled>Continue to next Claim</button>';

        }
        else{
            $lastpage=1;
            echo '<button  id = "btn_submit" class="btn" onClick="sendQueryWithAnswerLast('. $page_next .','. $workerid . ');" disabled>Continue to next Claim</button>';
        }
