...
def daytime(epoch):
	try:
		return datetime.date.fromtimestamp(epoch).isoweekday()
	except:
		return 0
...
def load_data(inputpath, nrows=None):
	...
	if inputpath.endswith(".dat.rar"):
		...
		try:
			sio = StringIO.StringIO(subprocess.check_output(cmd, stderr=fnull))
		except subprocess.CalledProcessError:
			print("somthing is wrong")  #continue
...
featsall_prediction_producer_global = [
	"dow",  # "favorite_count",  # "favourites_count",
	"followers_count",
	"friends_count",
	"lang",
	"pod",
...
featsall_prediction_producer_global_audiences = [
	"dow",  # "favorite_count",  # "favourites_count",
	"followers_count",
	"friends_count",
	"lang",
...
featsall_prediction_producer_global_text = [
	"dow",  # "favorite_count",  # "favourites_count",
	"followers_count",
	"friends_count",
	"lang",
...
featsall_prediction_nothing = [
	"followers_count",
	"friends_count",
	"lang",
]

featsall_prediction_basic = [
	"dow",
	"followers_count",
	"friends_count",
	"lang",
...
featsall_prediction_basic_text = [
	"dow",
	"followers_count",
	"friends_count",
	"lang",
...
def get_featnames_from_filename(filename, verbose=False):
	...
	featstotrain = []
	if "producer_global_audiences" in filename:
		featstotrain = featsall_prediction_producer_global_audiences
	elif "audience" in filename:
		featstotrain = featsall_prediction_audiences
	elif "only_text" in filename:
		featstotrain = featsall_prediction_only_text
	elif "nothing" in filename:
		featstotrain = featsall_prediction_nothing
	elif "basic_text" in filename:
		featstotrain = featsall_prediction_basic_text
	elif "producer_global_text" in filename:
		featstotrain = featsall_prediction_basic_text
	elif "producer_global" in filename:
		featstotrain = featsall_prediction_producer_global
	elif "producer" in filename:
		featstotrain = featsall_prediction_producer
	elif "global" in filename:
		featstotrain = featsall_prediction_global
	elif "basic" in filename:
		featstotrain = featsall_prediction_basic

	featstotrain = featsall_prediction_basic[:]
		# else:#  "social" in filename or "allfeats" in filename:
		# featstotrain += [
		#       ####tweet_id userid
		#       #"rt_above_med",# "followers_count",
		#       # "friends_count",
	...
	# ,"day"
	# #"lang"
	#       #listed_count    statuses_count ntweets	perc_en
	#       #"retweet_count_avg", "retweet_count_med"
	#      # 	retweet_count(rt)	favorite_count(fv)	in_reply_to_uid	in_reply_to_tid	retweeted_uid	created_at	lang	text
	#       ]
