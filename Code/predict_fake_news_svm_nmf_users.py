...
def my_split(df_in, row_n, thr):
   ...
   for i in range(52):
      ...
      if i==col_n:
         ...
         list_user = range(len(df_in.index.tolist()))
...
def my_split_users(df_in, col_n, thr):
   df_in
   total_c = 0
   train_rating_all = []
   train_user_all = []
...
# fdf_s = pd.read_csv('nmf_news.csv', sep="\t")
...
for fileInd in [1,2,3]:
   ...
   for news_ind in range(52):
      ...
      for test_run in range(5):
         df_train, df_test = my_split(ff_n, news_ind, .1)
         ff_n.to_csv('test_rating.csv', columns=ff_n.columns, sep="\t", index=False)
         ...
         df_train.to_csv('df_train.csv', columns=df_train.columns, sep="\t", index=False)
         df_test.to_csv('df_test.csv', columns=df_test.columns, sep="\t", index=False)
         ...
         training_data = tc.SFrame.read_csv('df_train.csv',delimiter='\t')
         test_data = tc.SFrame.read_csv('df_test.csv',delimiter='\t')
...
df_main.to_csv('nmf_news_users_10.csv', columns=df_main.columns, sep="\t", index=False)
