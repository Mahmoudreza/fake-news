...
def my_split(df_in, row_n, col_n):
   df_in
   total_c = 0
   train_rating_all = []
   train_user_all = []
...
def main():
   ...
   # fdf = pd.read_csv('nmf_news_cross_entropy_10.csv', sep="\t")
   ...
   for fileInd in [3]:
      ...
      ff_n_run = ff_n.copy()
      pred_assess_news = collections.defaultdict()
      real_assess_news = collections.defaultdict()
      real_big_pred_count = collections.defaultdict()
      real_eq_pred_count = collections.defaultdict()
      real_less_pred_count = collections.defaultdict()
      # ff_n_run = ff_n_dem
      # ff_n_run = ff_n_rep
      fdf = ff.copy()
      for news_ind in range(52):
         fdf.loc[:, 'nmf_p_label_' + str(news_ind)] = fdf['worker_id'] * 0.0
         ...
         real_eq_pred=0
         real_less_pred=0
         print('news_id: ----------------------------------------------------------------------------------------> :' + str(news_ind))
         for test_run in range(len(ff_n)):
         # for test_run in range(2):
            ...
            try:
               ...
               df_train, df_test = my_split(ff_n_run, test_run,news_ind)
               ...
               df_train.to_csv('df_train'+str(fileInd)+'.csv', columns=df_train.columns, sep="\t", index=False)
               df_test.to_csv('df_test'+str(fileInd)+'.csv', columns=df_test.columns, sep="\t", index=False)
               ...
               training_data = tc.SFrame.read_csv('df_train'+str(fileInd)+'.csv',delimiter='\t')
               test_data = tc.SFrame.read_csv('df_test'+str(fileInd)+'.csv',delimiter='\t')
               ...
               fdf['nmf_p_label_' + str(news_ind)][test_run]=y_pred
               y_true = test_data['rating'][0]
               pred_ans.append(y_pred)
               real_ans.append(y_true)
               # if y_true > y_pred:
               #    real_big_pred+=1
               # elif y_true==y_pred:
               #    real_eq_pred+=1
         ...
         pred_assess_news[news_ind] = np.mean(pred_ans)
         real_assess_news[news_ind] = np.mean(real_ans)
         real_big_pred_count[news_ind] = real_big_pred
         real_eq_pred_count[news_ind] = real_eq_pred
         real_less_pred_count[news_ind] = real_less_pred
      # for i in range(52):
      ...
      #    real_big_pred_list.append(real_big_pred_count[i])
      #    real_eq_pred_list.append(real_eq_pred_count[i])
      #    real_less_pred_list.append(real_less_pred_count[i])
      #    real_news_list.append(real_assess_news[i])
      #    pred_news_list.append(pred_assess_news[i])
   ...
   #                         'real_wsdm_news': Series(real_news_list, index=index_df),
   #                         'pred_wsdm_news': Series(pred_news_list, index=index_df),
   #                         'real_big_pred_count': Series(real_big_pred_list, index=index_df),
   #                         'real_eq_pred_count': Series(real_eq_pred_list, index=index_df),
   #                         'real_less_pred_count': Series(real_less_pred_list, index=index_df),
   #
   #                         })

   fdf.to_csv('nmf_prediction_news_' + str(fileInd)+ '.csv', columns=fdf.columns, sep="\t", index=False)
